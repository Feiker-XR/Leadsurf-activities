...
(function(undefined) {
	...
	var selectedNum = [],
		...
		var btn = $('.tab-content .pic a'),
			...
			left = ($(window).width() - 946)/2;
		
		btn.eq(0).bind('click', function() {
			//打开活动细则窗口
			window.open('prize.html#title1', 'newwindow','height=700,width=946,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')
		});
		btn.eq(1).bind('click', function() {
			//打开活动细则窗口
			window.open('prize.html#title2', 'newwindow','height=700,width=946,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')
		});
		btn.eq(2).bind('click', function() {
			//打开活动细则窗口
			window.open('prize.html#title3', 'newwindow','height=700,width=946,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')
		});
		btn.eq(3).bind('click', function() {
			//打开活动细则窗口
			window.open('prize.html#title4', 'newwindow','height=700,width=946,top='+top+',left='+left+',toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no')
		});
	},
		...
		mathTime = function() {
			var qiangpiaoT = new Date(qiangpiao).getTime(),
				...
				dom = $('.header-inner div:first');

			switch(true){
				case nowtime < qiangpiaoT:
				return qiangpiao;
				break;
				case nowtime < baomingT && nowtime > qiangpiaoT:
				dom.removeClass().addClass('timeline2');
				return baoming;
				break;
				case nowtime < qidongT && nowtime > baomingT:
				dom.removeClass().addClass('timeline3');
				return qidong;
				break;
				case nowtime < gongbuT && nowtime > qidongT:
