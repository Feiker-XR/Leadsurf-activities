    ...
    F.prototype = {
       		...
       		if( flag == 'init' ){       			
				var now_angle = 0;
	   			me.rotaryTimer = setInterval(function(){
	   				now_angle += 2;
                    /*
                        之所以不用这段代码是因为在ie下会把me当成timer的环境变量
                        导致me.$rotary无法获取，其值为null，这应该跟作用域有关
                    */
                    // me.$rotary.rotate(now_angle);
                    $('#rotary').rotate(now_angle);
		   		}, 150);
	   		}else if( flag == 'prepare' ){
	   			var now_angle = $('#rotary').getRotateAngle()[0];
	   			me.rotaryTimer = setInterval(function(){
	   				now_angle += 10;
	   				$('#rotary').rotate(now_angle);
		   		}, 80);
        ...
        createHistory: function(data){        	
        	var html = '<div class="rh-list">' +
					    	...
					    	'<span class="rh-desc">' +data.desc+ '奖金</span>' +
