$(function() {
	...
	rewards.prototype = {
		...
		action3: function(dom, step, i, res) {
			...
			dom.find('.money-amount').text(res['data']['balance']);
		},
		...
		action8: function(dom, step, i, res) {
			...
			dom.html(me.getHtml(8, data)).removeClass('.rp-step-8').addClass('rp-step-7');
			$contentDom = dom.find('.rp-content');
			$contentDom.css('opacity', 0);
			setTimeout(function() {
				$contentDom.css('opacity', 1);
