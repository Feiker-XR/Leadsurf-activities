    ...
    slotAnimation.prototype = {
      ...
      numberAnimation: function(fncallback){
        ...
        this.$number.each(function(index,ele){
          $(ele).delay(300*index).animate({
            marginTop : - (_this.height*10*1 + _this.arr[index]*_this.height)+'px'
          },1500,function(){
            if(_this.playMusic){
...
.factory('Countup', function($filter) {
...
.factory('Winning', function() {
    ...
    winAnimation.prototype = {
      ...
      start: function(bigwin){
        if(bigwin){
          this.bigwinStart();
        }
        this.creatCoin(bigwin);

      },
      creatCoin: function(bigwin){
        var _this = this;
        var forStart = 0;
        for(var i = 0; i<30; i++){
          var $coin = $('<span class="iconrmb"></span>');
          $coin.css({'left': Math.random()*100+'%','-webkit-transform':'rotate('+ Math.random()*360 +'deg)'+' '+ 'scale('+ (Math.random()*0.2+1) +')' })
          this.$dom.append($coin);
          $coin.delay(Math.random()*4000).animate({'top': $('#drawingContent').height()},2000,'easeInQuad',function(){
            forStart++;
            $(this).remove();
            if(forStart >= 30) {
              if(bigwin){
  ...
  .factory('Tip', function() {
    function tipAnimationo(word,dom) {
      this.$dom = dom || $('ion-content').find('.scroll');
      this.word = word || '提示';
    }
