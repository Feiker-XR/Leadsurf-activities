...
$(function(){
	...
	*/
	...
	//新列表生成
	function NewPic(tags,texts){
		oPics.each(function(){
			$(this).removeClass('current').stop().fadeOut(600);
			for(i=0;i<tags.length;i++){
				if(arrInclude(tags[i],texts)){
					$(oPics[i]).addClass('current');
				}
			}
		});
...
$(function(){
	...
	var oIcoRe = $('.ico-pop-recharge');
	var accountMain = $('#account-main');
	var accountSub = $('#account-sub');
	var oBtnPop = $('#pop-fund .btn');
	oBtnFund.click(function(){
		$('#pop-fund').pop();
	});
	oIcoRe.click(function(){
		var a1 = accountMain.html();
		var a2 = accountSub.html();
		accountMain.html(a2);
		accountSub.html(a1);
	});
	oBtnPop.click(function(){
...
$(function(){
	...
	userName.bind('blur', function(){
		...
		if(ret>0){
			prompt.show();
			check.hide();
			var url = "testSer?name="+$("#username").val();
