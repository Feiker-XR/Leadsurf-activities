...
(function(host, name, GameMethod, undefined){
	...
	var pros = {
		...
		//从服务器获取奖金等级配置
		loadBonusData: function(){
			var me = this,
				typeName = me.defConfig.name.split('.')[2];

			$.ajax({
				url:'simulatedata/bjkl8Bonus.php',
				data:{'type': typeName},
				dataType:'json',
				success: function(result){
					
					if(result['isSuccess'] == 1){
						me['bonusLevel'] = result['data'];
					}else{
						GameMessage.show({
							'mask':true,
							'content':'北京快乐8' + typeName + '玩法，奖金组配置读取失败，请重试。',
							'confirmIsShow':true,
							'confirmText':'重新读取',
							'confirmFun':function(){
								me.loadBonusData();
