$(document).ready(function(){
	...
	var Sort = {
		...
		currentName:'',
		currentLevel:'',
		html_input:'<div><input id="J-input-sortname" type="text" value="<#=value#>" class="input"/>限8个字符</div>',
		...
		add:function(id){
			...
			var html = id < 1 ? me.html_input :me.html_input.replace('限8个字符','限15个字符');
	...
	function doAdd(Sort,action){
		...
		if(v != name){
			//目录名称校验
			if(id < 1 && (WidthCheck(v) < 1 || WidthCheck(v) > 16)){
	...
	function doEdit(Sort,action){
		...
		if(v != name){
			//目录名称校验
			if(level == 1 && (v.length < 1 || v.length > 16)){
				alert('一级目录长度应为1-8个字符');
				return;
			}else if(v.length < 1 || v.length > 30){
				alert('二级目录长度应为1-15个字符');
				return;
			}
			jQuery.ajax({
