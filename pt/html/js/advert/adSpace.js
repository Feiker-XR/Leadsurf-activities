...
$(document).ready(function(){
	...
	$('#J-button-select-page').click(function(){
		...
		Wd.show({
			mask:true,
			title:'选择页面',
			content:$('#J-tpl-selectpages').text(),
			confirmIsShow:true,
			cancelIsShow:true,
			confirmFun:function(){
	...
	$('#J-button-select-params').click(function(){
			...
			success:function(data2){
				...
				if(false){
					...
					$.each(data2.body.result, function(){
						...
						strArr.push('<tr><td><input name="radioParam" type="radio" class="radio" value="'+ this.id +'"></td><td><span class="paramTitle">'+ this.name +'</span></td><td><span class="paramWidth">'+ w +'</span>*'+ h +'</td><td>'+ this.position +'</td><td>'+ this.pageMemo +'</td></tr>');
					});
					
					$('#J-tpl-selectparams-list').html($('#J-tpl-selectparams').text().replace(/<#=list#>/g, strArr.join('')));
			}
		});
		...
		Wd.show({
			mask:true,
			title:'选择参数',
			content:$('#J-tpl-selectparams-list').text(),
			confirmIsShow:true,
			cancelIsShow:true,
			confirmFun:function(){
