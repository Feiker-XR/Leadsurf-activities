...
function TableStyle(obj,count,colors,description){
    ...
    var newHeight = $("html").height() - $(".menu").height() - $(".crumbs").height() -$(".ui-tab-title").height()-20; // + "px";
   
	if($('.ui-tab-content').length>0){
		$('.ui-tab-content').css('overflow-x', 'hidden');
		if (contentHeight < newHeight)
		{
			$(".ui-form").css("height", newHeight + "px");
		}	
	}else{
		if($('.main').length>0){
