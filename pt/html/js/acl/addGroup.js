...
groupNameDom.blur(function(){
	...
	var buildLeft = function(data){
			...
			Tree.addEvent('click', function(e, el){
				var me = this,dom = $(el),action = dom.attr('data-action'),checkDom,id,node,par,childs,checkedNum = 0;
				if(action != 'check'){
					return;
				}
	...
	$('#J-button-submit').click(function(){
		...
		var pid = $("#select").val();
		var data="&pid="+pid+"&inUser="+inUser+"&name="+title+"&ids="+ids+"&userId=1"+"&num=" + Math.random();
